...
// LoadHistory replaces the current history buffer with the provided points.
// Used for restoring history from persistent storage on startup.
func LoadHistory(points []HistoryPoint) {
	histMu.Lock()
	defer histMu.Unlock()
	if len(points) > histCapacity {
		points = points[len(points)-histCapacity:]
	}
	histBuf = make([]HistoryPoint, len(points))
	copy(histBuf, points)
}
...
func GetDockerContainers() []DockerContainer {
	...
	// Pre-build base containers (non-running ones don't need stats)
	...
	if len(runningIndices) > 0 {
		...
		for _, idx := range runningIndices {
			wg.Add(1)
			go func(containerIdx int) {
				defer wg.Done()
				c := apiContainers[containerIdx]
				...
				if err != nil {
					slog.Debug("docker: failed to get stats", "container", c.ID[:12], "error", err)
					return
				}
				...
				if json.Unmarshal(statsBody, &stats) == nil {
					...
					if sysDelta > 0 && stats.CPUStats.OnlineCPUs > 0 {
						containers[containerIdx].CPUPct = (cpuDelta / sysDelta) * float64(stats.CPUStats.OnlineCPUs) * 100.0
					}
					containers[containerIdx].MemUsage = stats.MemoryStats.Usage
					containers[containerIdx].MemLimit = stats.MemoryStats.Limit
					mu.Unlock()
				}
			}(idx)
		}
