...
(function() {
  ...
  const renderDisks = (disks) => {
    ...
    for (let i = 0; i < disks.length; i++) {
      ...
      html += `<tr>
        <td data-label="Mount">${esc(d.mountpoint)}</td>
        <td data-label="Device">${esc(d.device)}</td>
        <td data-label="FS">${esc(d.fstype)}</td>
        <td data-label="Total">${fmtBytes(d.total)}</td>
        <td data-label="Used">${fmtBytes(d.used)}</td>
        <td data-label="Free">${fmtBytes(d.free)}</td>
        <td data-label="Use%" class="${pctColorClass(d.usedPercent)}">${d.usedPercent.toFixed(1)}%</td>
        <td data-label="Usage"><div class="mini-bar"><div class="bar-fill${cls}" style="width:${d.usedPercent.toFixed(0)}%"></div></div></td>
      </tr>`;
    }
  ...
  const renderNetwork = (nets) => {
    ...
    for (let i = 0; i < nets.length; i++) {
      const n = nets[i];
      html += `<tr>
        <td data-label="Interface">${esc(n.name)}</td>
        <td data-label="IP" style="max-width:200px;overflow:hidden;text-overflow:ellipsis">${esc(n.addrs || '-')}</td>
        <td data-label="Sent">${fmtBytes(n.bytesSent)}</td>
        <td data-label="Recv">${fmtBytes(n.bytesRecv)}</td>
        <td data-label="↑ Rate">${fmtRate(n.sendRate)}</td>
        <td data-label="↓ Rate">${fmtRate(n.recvRate)}</td>
      </tr>`;
    }
    tbody.innerHTML = html;
  };
  ...
  const renderProcesses = (procs) => {
    ...
    for (let i = 0; i < sorted.length; i++) {
      const p = sorted[i];
      html += `<tr>
        <td data-label="PID" class="col-pid">${p.pid}</td>
        <td data-label="Name">${esc(p.name)}</td>
        <td data-label="CPU%" class="col-num ${p.cpu > 50 ? 'c-red' : p.cpu > 20 ? 'c-yellow' : ''}">${p.cpu.toFixed(1)}</td>
        <td data-label="MEM%" class="col-num ${p.mem > 50 ? 'c-red' : p.mem > 20 ? 'c-yellow' : ''}">${p.mem.toFixed(1)}</td>
        <td data-label="Status" class="col-status">${statusLabel(p.status)}</td>
      </tr>`;
    }
    tbody.innerHTML = html;
  };
  ...
  const renderDocker = (containers) => {
    ...
    for (let i = 0; i < containers.length; i++) {
      ...
      html += `<tr>
        ...
        <td data-label="State"><span class="${badgeCls}">${esc(state)}</span></td>
        <td data-label="Status">${status}</td>
        <td data-label="CPU%" class="col-num">${cpuPct}</td>
        <td data-label="Memory" class="col-num">${memStr}</td>
  ...
  const render = (data) => {
    ...
    // Update process tree if on that tab
    const treeTab = document.getElementById('tab-tree');
    if (treeTab && treeTab.classList.contains('active') && data.processes) {
  ...
  const drawChart = () => {
    ...
    drawLine(colors.green, 'c');
    drawLine(colors.blue, 'm');
  };
  ...
  window.exportTrafficCSV = function() {
    const view = document.getElementById('traffic-view').value;
    const data = view === 'port' ? trafficData.ports : trafficData.hosts;
    ...
    let csv = '';
    if (view === 'port') {
      csv = 'Port,Total Bytes,Total Packets,Peak Rate,Average Rate\n';
      data.forEach(p => {
        csv += `${p.port},${p.total_bytes || 0},${p.total_packets || 0},${p.peak_rate || 0},${p.avg_rate || 0}\n`;
      });
    } else {
      csv = 'Host IP,Total Bytes,Total Packets,Peak Rate,Average Rate\n';
      data.forEach(h => {
        csv += `${h.host_ip},${h.total_bytes || 0},${h.total_packets || 0},${h.peak_rate || 0},${h.avg_rate || 0}\n`;
      });
    }
    ...
    link.setAttribute('download', `traffic-${view}-${timestamp}.csv`);
